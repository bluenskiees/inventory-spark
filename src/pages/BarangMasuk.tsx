...
import { ArrowDownCircle, Save, Plus, Trash2 } from "lucide-react";
import { Button } from "@/components/ui/button";
import { toast } from "sonner";
import { supabase } from "@/integrations/supabase/client";
import { useAuth } from "@/hooks/useAuth";

interface ItemRow { item_id: string; qty: string; satuan: string; keterangan: string; }
...
export default function BarangMasuk() {
  ...
  const [items, setItems] = useState<ItemRow[]>([{ item_id: "", qty: "", satuan: "", keterangan: "" }]);
  const [barangList, setBarangList] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    supabase.from("items").select("id, nama, satuan").order("nama").then(({ data }) => {
      if (data) setBarangList(data);
    });
  }, []);

  const addRow = () => setItems([...items, { item_id: "", qty: "", satuan: "", keterangan: "" }]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!supplier) { toast.error("Supplier wajib diisi"); return; }
    ...
    else { toast.success("Transaksi barang masuk berhasil!"); navigate("/transaksi/riwayat"); }
    setLoading(false);
  };
    ...
    <div className="animate-fade-in">
      <div className="mb-8">
        <h2 className="text-3xl font-bold flex items-center gap-2">
          <ArrowDownCircle className="h-8 w-8 text-success-foreground" /> Barang Masuk
        </h2>
        <p className="text-muted-foreground">Catat transaksi barang masuk ke gudang</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-6 max-w-4xl">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium mb-2">Supplier / Sumber</label>
              <input className={inputClass} placeholder="Nama supplier" value={supplier} onChange={(e) => setSupplier(e.target.value)} required />
            </div>
            <div>
                ...
                <div key={i} className="grid grid-cols-12 gap-3 items-end">
                  <div className="col-span-4">
                    <select className={inputClass} value={row.item_id} onChange={(e) => updateRow(i, "item_id", e.target.value)} required>
                      ...
                      {barangList.map((b: any) => <option key={b.id} value={b.id}>{b.nama}</option>)}
                    </select>
                  </div>
                  ...
                  <div className="col-span-3">
                    <input className={inputClass} placeholder="Keterangan" value={row.keterangan} onChange={(e) => updateRow(i, "keterangan", e.target.value)} />
                  </div>
